...
- Add an optional `API_PLATFORM_CACHE` variable to set the metadata/schema cache store separately from `CACHE_STORE`. Unset, it follows `CACHE_STORE`, so existing installs are unchanged.
